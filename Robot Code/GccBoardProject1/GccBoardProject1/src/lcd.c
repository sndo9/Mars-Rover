...
void lcd_init(void) 
{
	...
	PORTA|=0x00;  /// Setting display on, cursor on, blink off
	lcd_toggle_clear(1);
	PORTA|=0x0E;
	lcd_toggle_clear(1);
...
{							
	lcd_command(0xD4);		/// First bit relevant to the set DDRAM address instruction. The remaining bits designate address, which is 0x54.
}
